...
def likelihood_func(w, X, y_train, likelihood_var):
    ...
    likelihood = 1.
    mu = np.dot(X,w)
    for y, mean in zip(y_train.A1,mu.A1):
...
def get_posterior_params(X, y_train, prior, likelihood_var = 0.2**2):
    ...
    # TO DO
    post_mean = (np.dot(X.T, X)+ likelihood_var*prior['var'].I).I*np.dot(X.T, y_train)
    post_var = (likelihood_var**-1*np.dot(X.T,X)+prior['var'].I).I
